package SimpleBBS::Web::Plugin::Session;
use strict;
use warnings;
use utf8;
...
my $cipher = Crypt::CBC->new({
    key => '8dI9r0xvr4lFVnzY3l_ApawL_8XDmMdr',
    cipher => 'Rijndael',
});
sub _session {
    my $self = shift;
    ...
    if (!exists $self->{session}) {
        $self->{session} = HTTP::Session2::ClientStore2->new(
            env => $self->req->env,
            secret => '40pDJS7IxAJac5xxVuERCa-CiCoPfJKs',
            cipher => $cipher,
        );
    }
    return $self->{session};
...
This module manages session for SimpleBBS.
